...
use hardy_core::repair::{RepairProgress, RepairSummary};
...
impl HardyMonitorApp {
            ...
            Message::StartRepairJob => {
                ...
                Task::perform(
                    ...
                    |r| match r {
                        ...
                        Err(e) => Message::RepairCompleted(Err(AppError::from_anyhow_db(
                            e,
                            "repair_date_range",
                        ))),
                    },
        ...
        Task::perform(
            ...
            |r: Result<Vec<OccupancyLog>, anyhow::Error>| {
                Message::HistoryLoaded(
                    r.map_err(|e| AppError::from_anyhow_db(e, "get_history_range")),
                )
            },
        )
    }
    ...
    fn load_prediction_baseline(db: Arc<Database>, days: i64, clock: &dyn Clock) -> Task<Message> {
        ...
        Task::perform(
            ...
            |r: Result<Vec<HourlyAverage>, anyhow::Error>| {
                Message::PredictionBaselineLoaded(
                    r.map_err(|e| AppError::from_anyhow_db(e, "get_prediction_baseline")),
                )
            },
        )
    }
        ...
        Task::perform(
            ...
            |(current, baseline): (
                ...
                Message::InsightsDataLoaded {
                    current: current
                        .map_err(|e| AppError::from_anyhow_db(e, "get_insights_current")),
                    baseline: baseline
                        .map_err(|e| AppError::from_anyhow_db(e, "get_insights_baseline")),
                }
            },
        )
    }
